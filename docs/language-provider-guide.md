...
English for `en` / `en-*`, Chinese for `zh` / `zh-*`, Japanese for `ja` /
`ja-*`, then falls back to generic analysis.
...
compact lexicon there, and Japanese keeps script and suffix rules there.
