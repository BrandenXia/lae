#!/usr/bin/env python3
"""Render English text as Markdown using learned partial-word emphasis."""
...
DEFAULT_MODEL = ROOT / "models" / "lae-provo-segmental-v1.lem"
...
def markdown(source: str, spans: tuple) -> str:
    ...
    with Runtime(_runtime_path(arguments.library)) as runtime:
        with runtime.load_model_file(arguments.model) as model:
            emphasis = runtime.process(
                paragraph,
                ProcessOptions(
                    language="en", salience_threshold=arguments.threshold
                ),
                model=model,
            )
    print(markdown(paragraph, emphasis))
